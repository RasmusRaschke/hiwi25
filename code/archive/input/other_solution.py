...
def f(t,x,system_para,epsilon= 0.01):
    ...
    grav_torque = -I*R/(I+m*R**2)*np.cross(grav_force,n)

    total_torque = grav_torque - np.cross(magnetic_moment(alpha,beta,mu),B)

    omega = L/I

    roll_resist_dir = np.array([0.0,0,0])
    if np.linalg.norm(omega) > epsilon:
        roll_resist_dir = -omega/np.linalg.norm(omega)
    elif np.linalg.norm(total_torque) > epsilon:
        roll_resist_dir = -total_torque/np.linalg.norm(total_torque)
    else:
        roll_resist_dir = np.array([0.0,0,0])

    normal_gravity = -np.dot(grav_force,n)
    roll_resist = eta*roll_resist_dir*R*normal_gravity

    # print(np.linalg.norm(roll_resist),np.linalg.norm(total_torque))
    if np.linalg.norm(omega) < epsilon and np.linalg.norm(roll_resist) > np.linalg.norm(total_torque):
        torque = np.array([0.0,0,0])
    else:
        torque = total_torque+roll_resist
    ...
    return np.array([dx,dy,dalpha,dbeta,torque[0],torque[1],torque[2]])
...
if __name__ == "__main__":
    ...
    t_max = system_para[5]
    n_eval = 1000
    x_0 = np.array([0.0,0.0,alpha,beta,0.0,0.0,0.0])
    sol = solve_ivp(f,[0,t_max],x_0,args=(system_para,),t_eval=np.linspace(0,t_max,n_eval),method='RK45',rtol=1e-6,atol=1e-6)
