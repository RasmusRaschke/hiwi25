...
datasets = [
    ...
    {
        "name": "2.5",
        "x": data["sample5"].x, "y": data["sample5"].y, "z": data["sample5"].z,
        "mu_x": data["sample5"].mu_x, "mu_y": data["sample5"].mu_y, "mu_z": data["sample5"].mu_z,
        "color": YELLOW,
    },
    {
        "name": "-1.9",
        "x": data["sample9"].x, "y": data["sample9"].y, "z": data["sample9"].z,
        "mu_x": data["sample9"].mu_x, "mu_y": data["sample9"].mu_y, "mu_z": data["sample9"].mu_z,
        "color": RED,
    },
    {
        "name": "-2.5",
        "x": data["sample10"].x, "y": data["sample10"].y, "z": data["sample10"].z,
        "mu_x": data["sample10"].mu_x, "mu_y": data["sample10"].mu_y, "mu_z": data["sample10"].mu_z,
        "color": PINK,
    },
    {
        "name": "-3.1",
        "x": data["sample11"].x, "y": data["sample11"].y, "z": data["sample11"].z,
        "mu_x": data["sample11"].mu_x, "mu_y": data["sample11"].mu_y, "mu_z": data["sample11"].mu_z,
        "color": GREEN,
    },
...
MU_TRAIL_DURATION = 0.5
SHOW_TIME_TRACKER = False
ZOOM = 0.7
ANIMATION_END = 1.00  # seconds
# ----------------------------
# Interpolators
# ----------------------------
for d in datasets:
...
class Anim(ThreeDScene):
    def construct(self):
        ...
        if EXPORT_STILL:
            ...
            self.play(
                time.animate.set_value(ANIMATION_END), run_time=2, rate_func=linear
            )
