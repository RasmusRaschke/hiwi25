...
std::string remove_comment(string line) {
...
T get_input_variable(const string& line) {
    string cleaned_line = remove_comment(line);
    try {
        if constexpr (is_same_v<T, double>) {
            return stod(cleaned_line);
        } else if constexpr (is_same_v<T, int>) {
            return stoi(cleaned_line);
        } else {
...
static vec3 getOmega(const vec3 &Omega, const quat &q, const Parameters &p){
    ...
    double vpar = (p.R * Omega.cross(p.n)).dot(t);
    double sgn = 0.0;
...
int main(){
    ...
    p.mu_body = vec3(get_next_input_variable<double>(), get_next_input_variable<double>(), get_next_input_variable<double>());
    p.B_inert = vec3(get_next_input_variable<double>(), get_next_input_variable<double>(), get_next_input_variable<double>());
    p.I = (2.0 / 5.0) * p.M * p.R * p.R;

    State s;
    s.r = vec3(0.0, 0.0, p.R*(1.0 - p.n.dot(vec3(0,0,1))));
    s.Omega = vec3(0.0, 0.0, 0.0);
    s.q = quat::Identity();
