...
def main() -> None:
    ...
    axs[0, 1].set_title("Planar trajectory")
    ...
    components(
        axs[2, 0], t, data,
        ("U_generic", "U_gr", "U_em"),
        (r"$U_{\mathrm{generic}}$", r"$U_g$", r"$U_{\mathrm{em}}$"),
        "Potential-energy contributions",
        r"$[\mathrm{J}]$",
    )

    axs[2, 1].plot(t, energy, label=r"$E_{\mathrm{total}}$")
    axs[2, 1].set_title("Total mechanical energy")
    axs[2, 1].set_xlabel(r"$t\,[\mathrm{s}]$")
    axs[2, 1].set_ylabel(r"$[\mathrm{J}]$")

    axs[2, 2].plot(t, relative_energy_drift, label=r"$(E-E_0)/|E_0|$")
