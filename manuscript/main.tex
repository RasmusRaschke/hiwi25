...
\end{equation}
where $\hat{\bm{\mu}}(t):=P(t)\bm{\mu}_0$, $I_0$ is the inertia scalar of an ideal ball, and $g$ is the gravitational acceleration. We obtain a Lagrangian $L(z,\bm{\Omega},\dot{\bm{r}}) := T - U$. Assuming that there is an external force $\bm{F}_\text{e}=\bm{F}_\text{e}(\bm{r})$ such as friction, the corresponding Lagrange-d'Alembert principle reads
\begin{equation}
...
\] where $\bm{\tau}_\text{em}:=\hat{\bm{\mu}}(t) \times \bm{B}$ is the magnetic torque, and $\bm{\tau}_\text{e} := -R \bm{n} \times \bm{F}_\text{e}$ is the torque induced by external forces.\cite{Crepel2005}\cite{Lagrange1788}
...
\end{equation}
equations for the angular acceleration from Equation \ref{eq:lagrangealembert}. Taking the derivative of Equation \ref{eq:rollingconst}, this fully determines the linear motion. Assuming Coulomb friction\cite{Coulomb}\cite{Hayakawa2005} and aerodynamical drag by the drag equation, the external force becomes \begin{equation}
    \label{eq:friction}
    \bm{F}_\text{e} = - \nu_\text{r} Mg \langle \bm{n},\bm{e}_z\rangle \sgn (\dot{\bm{r}}) + \frac{\rho C A}{2} \|\dot{\bm{r}}\|^2
    ,\end{equation} where $\nu_\text{r}$ is the rolling resistance coefficient, $\sgn$ is the sign function, $\rho$ is the fluid density of air, $C$ is the drag coefficient, and $A$ is the cross-sectional area of the sphere.\cite{Coulomb}\cite{Hayakawa2005}\cite{Landau2002} For numerical reasons, we reparametrize $P(t)=P((q_0(t),\bm{q}(t)))$, where \[
...
Assuming the International Standard Atmosphere and moderate velocities, values of $\rho = 1.2 \, \text{kgm}^{-3}$ and $C=0.47$ were used for simulations.\cite{iso2533}\cite{Baker1983} 
\section{Results}
\subsection{Influence of the magnetic moment}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{f1.pdf}
    \caption{Final deviation $x|_{t=1 \, \text{s}}$ in relation to polar angle $\theta$ and modulus $\mu_0$. Each graph is associated with a fixed modulus $\mu_0$, where (a) shows curves for moduli below $1 \, \text{Am}^2$ and (b) for moduli including and above $1 \, \text{Am}^2$. In the small $\mu_0$-regime (a), the final derviation increases in magnitude with $\theta$ and is antisymmetric around the origin. The high $\mu_0$-regime (b) displays similar antisymmetry, yet different behaviour for high magnitudes of $\theta$. With increasing modulus, critical points shift towards lower absolute values of $\theta$, after which the curves behave erratic.}
    \label{fig:f1}
\end{figure}
\begin{figure}[H]
    \centering
...
\]  with $\mu_0 := \|\bm{\mu}_0\|$ and the polar angle $\theta$ defined as depicted in Figure \ref{fig:f1}. We chose the value of the $x$-coordinate after $1 \, \text{s}$ as measure for the deviation of the actual trajectory from a straight downhill path. The results are also shown in Figure \ref{fig:f1}. Antisymmetric behaviour in $\theta$ is predicted by $\bm{\mu}(-\theta)=-\bm{\mu}(\theta).$ The deviation from the straight path grows non-linearly with $\mu_0$ until a critical point is reached. Figure \ref{fig:f2} shows representative trajectories demonstrating that the curvature of the trajectories increases with $\mu_0$ and $\theta$.
\section{References}
