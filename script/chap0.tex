...
\begin{eg}
    Bridging to our discussion of $\SO(3)$, we demonstrate how to calculate the Lie algebra $\mathfrak{s}\mathfrak{o}(3)$. By the preceeding theorem, the Lie algebra is isomorphic to a tangent vector at the identity matrix $\id \in \SO(3)$. Hence, choose a smooth path \[
    \mathbb{R} \ni t \mapsto A(t) \in \SO(3)
    \] with $A(0)=\id$. We can expand up to first order as
    \[
    A(t) = \id + B(t)t + \mathcal{O}(t^2)
\] such that $\dot{A}|_{t=0} = B(0)$. 
\end{eg}
